...
# Функция для проверки занят ли логие при регистрации
...
# Функция для проверки ID при создании новой игры
...
def database(state, info=None):
    ...
    if state == "register":
        ...
        for i in result:
            cur.execute("SELECT (win, lose, draw) FROM statistic WHERE user_id = {}".format(i[0]))
            result2 = [n[0].replace("(", "").replace(")", "").split(",") for n in cur.fetchall()][0]
            res.append(i + result2)
...
def login():
    ...
    if form.validate_on_submit():
        ...
        # Проверка существуют ли такой логие и пароль
...
def main(info):
    ...
    # Создаем копию словаря с изночальными позициями фишками
    black = black_orig.copy()
    white = white_orig.copy()

    # Меняем позиции фишек если игра не закончена
    for i in moves:
        temp = i.strip().split(" ")
        if temp[0] == "d":
            try:
    ...
    # При закрытии игры удаляем из базы и обновляем статистику
...
def profile(username):
    ...
    # Проверяем если есть уже созданная игра
    if room == []:
        state = 0
        apponent = 0
